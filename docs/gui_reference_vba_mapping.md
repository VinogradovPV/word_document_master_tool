...
| Статус и действия | Обработать файлы | Нижняя панель, кнопка `Выполнить выбранные операции` | `SourceProcessingService` | partial | Глобальная кнопка учитывает настройки со всех вкладок. |
| Статус и действия | Объединить | Нижняя панель, кнопка `Объединить Word-документы` | `WordMergeService` | partial | Глобальная кнопка зависит от Word COM и настроек. |
...
| Статус и действия | План выполнения | Вкладка `Главное`, блок `План выполнения` | `build_execution_plan` | implemented | Показывает операции, ошибки и предупреждения до запуска. |
